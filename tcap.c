...
static void tcapmove(int row, int col) { putpad(tgoto(CM, col, row)); }

static void tcapeeol(void) { putpad(CE); }

static void tcapeeop(void) { putpad(CL); }
...
static void tcapbeep(void) { ttputc(BEL); }

static void putpad(char *str) { tputs(str, 1, ttputc); }
#endif /* TERMCAP */
