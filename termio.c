...
void ttopen(void) {
  ...
  if (status != SS$_NORMAL)
    exit(status);
  status = SYS$QIOW(EFN, iochan, IO$_SENSEMODE, iosb, 0, 0, oldmode,
...
ttgetc() {
  ...
  while (ibufi >= nibuf) {
    ...
    term[0] = 0;
    term[1] = 0;
    status = SYS$QIOW(EFN, iochan, IO$_READLBLK | IO$M_TIMED, iosb, 0, 0, ibuf,
                      NIBUF, 0, term, 0, 0);
    if (status != SS$_NORMAL)
      exit(status);
    status = iosb[0] & 0xFFFF;
    ...
    if (nibuf == 0) {
      status = SYS$QIOW(EFN, iochan, IO$_READLBLK, iosb, 0, 0, ibuf, 1, 0, term,
                        0, 0);
