...
start_over:
  ...
  for (;;) { /* ISearch per character loop */
    ...
    if (cpos >=
        NPAT) { /* too many chars in string?  */ /* Yup.  Complain about it */
 ...
 */
int checknext(char chr, char *patrn,
              int dir) /* Check next character in search string */
{
  ...
  if (dir > 0) {                      /* If searching forward                 */
    ...
    if ((status = eq(buffchar, chr)) !=
        0) {                   /* Is it what we're looking for?      */
 ...
 */
int match_pat(
    char *patrn) /* See if the pattern string matches string at "."   */
{
